...
#define SERVICE_NAME    "NtesUSBMon"
/** Win32 Device name. */
#define DEVICE_NAME     "\\\\.\\NtesUSBMon"
/** NT Device name. */
#define DEVICE_NAME_NT   L"\\Device\\NtesUSBMon"
/** Win32 Symlink name. */
#define DEVICE_NAME_DOS  L"\\DosDevices\\NtesUSBMon"
...
int __cdecl main(int argc, char **argv)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTPathAppend(szInfFile, sizeof(szInfFile), "NtesUSB.inf");
        }
...
int usblibOsCreateService(void)
{
    ...
    if (hSMgrCreate)
    {
        ...
        rc = RTPathExecDir(szDriver, sizeof(szDriver) - sizeof("\\NtesUSBMon.sys"));
        if (RT_SUCCESS(rc))
        {
            strcat(szDriver, "\\NtesUSBMon.sys");
