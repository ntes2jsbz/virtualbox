...
static int usbLibVuDevicePopulate(PVBOXUSB_DEV pVuDev, HDEVINFO hDevInfo, PSP_DEVICE_INTERFACE_DATA pIfData)
{
    ...
    do
    {
        ...
        LogRelFunc(("Found NtesUSB on `%s' (rc=%d)\n", pVuDev->szName, rc));
        AssertRC(rc);
    } while (0);
...
/* Enumerate proxied USB devices (with NtesUSB.sys loaded). */
 ...
 * least prior to capturing (once NtesUSB.sys is installed, a DriverKey must by definition be
