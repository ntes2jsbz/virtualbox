 ...
 *
 *
 * The USB Monitor (NtesUSBMon.sys) serves to capture and uncapture USB
 * devices. Its job is to ensure that the USB proxy (NtesUSB.sys) gets installed
 ...
 * NtesUSB.sys, the USB proxy. A typical solution for installing such USB proxy
 * is using a filter driver, but that approach was rejected because filter drivers
 * cannot be dynamically added and removed. What NtesUSBMon does instead is hook
 ...
 * that they return the synthetic USB VID/PID that NtesUSB.sys handles rather than
 * the true hardware VID/PID. That causes Windows to install NtesUSB.sys for the
 * device.
 *
 ...
 * filter matching criteria are morphed (see above) such that NtesUSB.sys loads
 * for them before any default driver does.
 *
 * There is an IDC interface to NtesUSB.sys which allows the USB proxy to report
 ...
 * and cycling the port). NtesUSBMon.sys relies on these IDC calls to track
 * captured devices and be informed when NtesUSB.sys unloads.
 *
 ...
 * requests and NtesUSBMon can't read any of their descriptors (note that in
 ...
 * should remain the same for the lifetime of a device. Note that NtesUSB.sys
 ...
 */
/*
  BusQueryDeviceID USB\Vid_81EE&Pid_CAFE
  BusQueryInstanceID 2
  BusQueryHardwareIDs USB\Vid_81EE&Pid_CAFE&Rev_0100
  BusQueryHardwareIDs USB\Vid_81EE&Pid_CAFE
...
static WCHAR const g_szBusQueryDeviceId[]      = L"USB\\Vid_81EE&Pid_CAFE";
static WCHAR const g_szBusQueryHardwareIDs[]   = L"USB\\Vid_81EE&Pid_CAFE&Rev_0100\0USB\\Vid_81EE&Pid_CAFE\0\0";
...
{
    RT_NOREF1(pDrvObj);
    LOG(("NtesUSBMonUnload pDrvObj (0x%p)", pDrvObj));
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                LOG(("NtesUSBMon::DriverEntry returning STATUS_SUCCESS"));
                return STATUS_SUCCESS;
            }
